...
const googleTranslateText = async (
  ...
  try {
    console.log(
      `🌍 Google Translate: ${sourceLanguage || "auto"} → ${targetLanguage}`
    );
    ...
    // Only send source when user selected one.
    // If omitted, Google can detect the source language.
    if (sourceLanguage) {
    ...
    return {
      ...
      detectedSourceLanguage:
        response.data?.data?.translations?.[0]
          ?.detectedLanguageCode || sourceLanguage || null,
    };
  } catch (err) {
    console.error(
...
app.post("/api/translate", async (req, res) => {
  ...
  console.log("🌍 Sarvam Translation");
  ...
  if (!process.env.SARVAM_API_KEY) {
    ...
    return res.status(500).json({
      success: false,
      error: "Sarvam API key is not configured on server",
    });
  }
  ...
  try {
    ...
    const translatedText =
      sarvamResponse.data?.translated_text;
